const TaxConstantTi = 0.035
const TaxConstantTr = 0.06
...
function calFee(IntelCenterLvl = 0, Hideoutmanagement = 0, baseValue,fleaPrice){
    ...
    */
    ...
    {
        baseFee = (baseValue * TaxConstantTi * Math.pow(4, Math.log10(baseValue / fleaPrice)) * count) + (fleaPrice * TaxConstantTr * Math.pow(4, Math.pow(Math.log10(fleaPrice / baseValue), LogTaxConstant)) * count)
    }
    else
    {
        baseFee = (baseValue * TaxConstantTi * Math.pow(4, Math.pow(Math.log10(baseValue / fleaPrice), LogTaxConstant)) * count) + (fleaPrice * TaxConstantTr * Math.pow(4, Math.log10(fleaPrice / baseValue)) * count)
    }
    return Math.round(baseFee)//Math.round(baseFee - ((baseFee * commission) / 100))
}
